...
import { Plus, Edit, Trash2, ShoppingCart, Search, RefreshCw, Package, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-text-muted mt-1">
            Manage your inventory, veterinary clinical specs, sizes, and pricing in real-time.
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        <div className="relative flex-1 max-w-md">
          <Search size={16} className="absolute left-3.5 text-text-muted" style={{ top: '50%', transform: 'translateY(-50%)' }} />
          <input
            type="text"
              ...
              <tbody className="divide-y divide-secondary/20">
                {filtered.map((product) => {
                  ...
                  const priceLabel = hasMultiple
                    ? `From Rs. ${Math.min(...productVariants.map((v) => Number(v.price)), 0).toLocaleString()}`
                    : productVariants[0]
                    ? formatPrice(productVariants[0].price)
                    : 'N/A';

                  return (
                    <tr key={product.id} className="hover:bg-secondary/15 transition-colors group">
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="max-w-[220px] md:max-w-xs">
                            <span className="font-bold text-text block truncate group-hover:text-accent transition-colors" title={product.name}>
                              {product.name}
                            </span>
                            {product.packaging && (
                      ...
                      <td className="py-3 px-3">
                        <span className={`badge ${product.is_featured ? 'badge-accent' : 'badge-default'} text-[9px] uppercase tracking-wider font-bold`}>
                      ...
                      <td className="py-3 px-3 text-right">
                        <div className="flex gap-1.5 justify-end">
                          <Link
                            ...
                            className="p-2 hover:bg-accent/15 hover:text-accent rounded-xl transition-colors text-text-muted"
                            title="Edit Product"
                          >
                            <Edit size={15} />
                          </Link>
