...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Error inserting product. Check for duplicate product names.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-heading font-extrabold text-2xl text-text flex items-center gap-2">
            <ShoppingCart size={24} className="text-accent" /> Add New Product
          </h1>
          <p className="text-xs text-text-muted mt-1">
            Create a new product listing in your catalog database.
          </p>
        </div>
      </div>
